...
class Routing:
    ...
    def __init__(self, path: str, app: ApplicationLike, *, document: bool = False, autoload: bool = False) -> None:
        self.path = path
        self.app = self._fixup(self._normalize(app))
        self.app_context = ApplicationContext(self.app, url=self.path)
...
def document(path: str, app: ApplicationLike) -> Routing:
    return Routing(path, app, document=True)

def autoload(path: str, app: ApplicationLike) -> Routing:
    return Routing(path, app, autoload=True)
...
def directory(*apps_paths: Path) -> List[Routing]:
    ...
    return [ document(path, app) for url, app in build_single_handler_applications(paths).items() ]
...
class RoutingConfiguration:
    ...
    def get_http_urlpatterns(self) -> List[URLPattern]:
        return self._http_urlpatterns + [path("/", AsgiHandler)]
    ...
    def _add_new_routing(self, routing: Routing) -> None:
        ...
        def urlpattern(suffix=""):
            return r"^{}$".format(join(re.escape(routing.path)) + suffix)

        if routing.document:
            self._http_urlpatterns.append(path(urlpattern(), DocConsumer, kwargs=kwargs))
        if routing.autoload:
            self._http_urlpatterns.append(path(urlpattern("/autoload.js"), AutoloadJsConsumer, kwargs=kwargs))

        self._websocket_urlpatterns.append(path(urlpattern("/ws"), WSConsumer, kwargs=kwargs))
